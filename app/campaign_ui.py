...
# === CONFIG & GLOBALS ===
BASE_TRACK_URL    = "https://tracker-y4fl.onrender.com"
SHEET_NAME        = "EmailStatus"
SHEET_NAME2       = "EmailTRACKV2"
SERVICE_ACCOUNT   = "service-account.json"
STATE_FILE        = "config/campaign_state.json"
SENDGRID_API_KEY  = os.getenv("SENDGRID_API_KEY", "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")
OPEN_AI_KEY='FdR4XViPRXJAUpLhdKnJnvXgX6vn7OPMDLYkjGXHBDPn8k5kmx1gJQQJ99BDACHYHv6XJ3w3AAABACOGkFao'
OPEN_AI_ENDPOINT='https://copyliteiq-ai-01.openai.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2025-01-01-preview'
client = AzureOpenAI(
    api_key=OPEN_AI_KEY,
...
with open("app/sender_config.json") as f:
    SENDERS = json.load(f)
...
def load_state():
    if os.path.exists(STATE_FILE):
        try:
            ...
            return {
                "campaign_row_state": raw.get("campaign_row_state", raw.get("client_index", 0)),
                "campaign_flag": raw.get("campaign_flag", raw.get("sender_index", 0))
            }
        except:
            logger.warning("Bad state file, resetting")
...
if preview:
    try:
        ...
        with open("config/email_template_analytics.html") as f:
            tmpl = f.read()
...
def send_batch():
    ...
    for _ in range(batch_size):
        ...
        subj = subject
        transport = sender.get("transport", "smtp").lower()
        ok = False
        status = "Failed"
